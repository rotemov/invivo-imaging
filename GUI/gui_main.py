...
def main():
    main_runner = [
        ...
        [sg.Text('Cut off point %', size=LABEL_SIZE),
         ...
         sg.Text("Recommended: 80-99\nIncrease to decrease superpixel size and clean edges")],
        [sg.Text('Correlation threshold fix %', size=LABEL_SIZE),
         sg.Slider(range=(1, 99), orientation='h', size=SLIDER_SIZE, key='corr_th_fix', default_value=45),
         sg.Text("Recommended: 30-60\nIncrease to decrease ROI size and clean edges")],
        [sg.Text('# bg elements', size=LABEL_SIZE),
         sg.Slider(range=(0, 20), orientation='h', size=SLIDER_SIZE, key='bg_rank', default_value=4), sg.Text("Recommended: 3-6\nIncrease if there are non cell ROIs")],
        [copy.deepcopy(SPACE)],
    ...
    advanced_params = [
        ...
        [sg.Text('Merge correlation threshold', size=LABEL_SIZE),
         ...
         sg.Text("Range: 0.01 - 0.99. Increase to separate grouped cells")],
        [sg.Checkbox('Binning', size=CHECK_BOX_SIZE, default=False, key="binning_flag"),
         sg.Text("Use for very large FOVs to decrease runtime")],
    ...
    super_pixels = [
        ...
        [sg.Text('Remove dimmest ', size=LABEL_SIZE),
         ...
         sg.Text("The numbers are in ascending order from brightest to dimmest. Warning: do not remove more than exist")],
    ]
