...
def noise_estimator(Y, noise_range=[0.25, 0.5], noise_method='logmexp', max_num_samples_fft=4000,
    ...
    if T > max_num_samples_fft:
        Y = np.concatenate((Y[..., 1:max_num_samples_fft // 3 + 1],
                            Y[...,
                            np.int(T // 2 - max_num_samples_fft / 3 / 2):np.int(T // 2 + max_num_samples_fft / 3 / 2)],
...
def threshold_data(Yd, th=2):
    ...
    for array in [Yd]:
        ...
        for i in range(dims[2]):
            Yt[:, :, i] = np.clip(array[:, :, i], a_min=(Yd_median + th * Yd_mad)[:, :, 0], a_max=None) - (
                                                                                                                  Yd_median + th * Yd_mad)[
                                                                                                          :, :, 0]
    return Yt
...
def ls_solve_ac(X, U, V, mask=None, beta_LS=None):
    ...
    for ii in range(K):
        if mask is None:  ## for update temporal component c
            beta_LS[[ii], :] = np.maximum(0, beta_LS[[ii], :] + (
                    (UK[[ii], :] - np.matmul(VK[[ii], :], beta_LS)) / aa[ii]));
        else:
            ind = (mask[ii, :] > 0);  ## for update spatial component a
            beta_LS[[ii], ind] = np.maximum(0, beta_LS[[ii], ind] + (
                    (UK[[ii], ind] - np.matmul(VK[[ii], :], beta_LS[:, ind])) / aa[ii]));
    return beta_LS
    ...
    for ii in range(mask_a.shape[-1]):
        for kk in range(num_plane):
            ...
            if mask_a[:, :, kk, ii].sum() > 0:
                while jj <= times:
                    labeled_array, num_features = scipy.ndimage.measurements.label(corr_img_all_r[:, :, kk, ii] > corr,
                                                                                   structure=s);
                    u, indices, counts = np.unique(labeled_array * mask_a[:, :, kk, ii], return_inverse=True,
                                                   return_counts=True);
                    # print(u);
                    if len(u) == 1:
...
def merge_components(a, c, corr_img_all_r, U, V, normalize_factor, num_list, patch_size, merge_corr_thr=0.6,
                     merge_overlap_thr=0.6, plot_en=False):
    ...
    cri = np.asarray((cor_corr / (temp.T)) > merge_overlap_thr) * np.asarray((cor_corr / temp) > merge_overlap_thr) * (
        (a_corr > 0).toarray());
    a = a.toarray();
    ...
    if len(connect_comps[0]) > 0:
        ...
        for comp in comps:
            ...
            a_temp = a[:, comp];
            if plot_en:
                spatial_comp_plot(a_temp, corr_img_all_r[:, comp].reshape(patch_size[0], patch_size[1], -1, order="F"),
                                  num_list[comp], ini=False);
...
def update_AC_l2(U, V, normalize_factor, a, c, b, patch_size, corr_th_fix,
    ...
    for iters in range(maxiter):
        ...
        temp = (a.sum(axis=0) == 0);
        if sum(temp):
            a, c, corr_img_all_r, mask_a, num_list = delete_comp(a, c, corr_img_all_r, mask_a, num_list, temp,
                                                                 "zero a!", plot_en);
        ...
        if update_after and ((iters + 1) % update_after == 0):
            corr_img_all = vcorrcoef(U / normalize_factor, V.T, c);
            rlt = merge_components(a, c, corr_img_all, U, V, normalize_factor, num_list, patch_size,
                                   merge_corr_thr=merge_corr_thr, merge_overlap_thr=merge_overlap_thr, plot_en=plot_en);
            ...
            mask_a = make_mask(corr_img_all_r, corr_th_fix, mask_a, num_plane,
                               max_allow_neuron_size=max_allow_neuron_size);
...
def update_AC_bg_l2(U, V, normalize_factor, a, c, b, ff, fb, patch_size, corr_th_fix,
    ...
    for iters in range(maxiter):
        ...
        temp = ls_solve_ac(np.hstack((c, ff)), np.hstack((V, -1 * f)), np.hstack((U, b)), mask=mask_ab.T,
                           beta_LS=np.hstack((a, fb))).T;
        a = temp[:, :-num_bg];
        fb = temp[:, -num_bg:];
        ...
        temp = ls_solve_acc(np.hstack((a, fb)), np.hstack((U, b)), np.hstack((V, -1 * f)), mask=None,
                            beta_LS=np.hstack((c, ff))).T;
        c = temp[:, :-num_bg];
        ff = temp[:, -num_bg:];
...
def reconstruct(Yd, spatial_components, temporal_components, background_components, fb=None, ff=None):
    ...
    dims = y0.shape;
    if fb is not None:
        mov_res = y0 - (np.matmul(spatial_components, temporal_components.T) + np.matmul(fb,
                                                                                         ff.T) + background_components).reshape(
            dims, order='F');
    else:
        mov_res = y0 - (np.matmul(spatial_components, temporal_components.T) + background_components).reshape(dims,
                                                                                                              order='F');
    return mov_res
...
def axon_pipeline(Yd, U, V, cut_off_point=[0.95, 0.9], length_cut=[15, 10], length_max=[100000, 100000], th=[2, 1],
                  pass_num=1, residual_cut=[0.6, 0.6],
	...
	*** parameters for plot: ***
    ...
    while ii < pass_num:
        ...
        if num_plane > 1:
            print("3d data!");
            connect_mat_1, idx, comps, permute_col = find_superpixel_3d(Yt, num_plane, cut_off_point[ii],
                                                                        length_cut[ii], length_max[ii],
            ...
            connect_mat_1, idx, comps, permute_col = find_superpixel(Yt, cut_off_point[ii], length_cut[ii],
                                                                     length_max[ii], eight_neighbours=True);
        print(time.time() - start);
        ...
        if ii > 0:
            a_ini, c_ini, brightness_rank = prepare_iteration(Yd_res, connect_mat_1, permute_col, pure_pix, a_ini,
                                                              c_ini);
            a = np.hstack((a, a_ini));
            c = np.hstack((c, c_ini));
        else:
            a, c, b, normalize_factor, brightness_rank = prepare_iteration(Yd, connect_mat_1, permute_col, pure_pix,
                                                                           a_ini, c_ini, more=True);
        print(time.time() - start);

        if plot_en:
            Cnt = local_correlations_fft(Yt);
            pure_superpixel_corr_compare_plot(connect_mat_1, unique_pix, pure_pix, brightness_rank_sup, brightness_rank,
                                              Cnt, text);
        ...
        if bg:
            a, c, b, fb, ff, res, corr_img_all_r, num_list = update_AC_bg_l2(U, V, normalize_factor, a, c, b, ff, fb,
                                                                             dims,
                                                                             corr_th_fix, maxiter=maxiter, tol=1e-8,
                                                                             update_after=update_after,
                                                                             merge_corr_thr=merge_corr_thr,
                                                                             merge_overlap_thr=merge_overlap_thr,
                                                                             num_plane=num_plane, plot_en=plot_en,
                                                                             max_allow_neuron_size=max_allow_neuron_size);
        ...
        superpixel_rlt.append(
            {'connect_mat_1': connect_mat_1, 'pure_pix': pure_pix, 'brightness_rank': brightness_rank});
        if pass_num > 1 and ii == 0:
            rlt = {'a': a, 'c': c, 'b': b, "fb": fb, "ff": ff, 'res': res, 'corr_img_all_r': corr_img_all_r,
                   'num_list': num_list};
            a0 = a.copy();
        ii = ii + 1;
    start_time = time.time();
    ...
    if plot_en:
        ...
        plt.show();
    fin_rlt = {'a': a, 'c': c, 'c_tf': c_tf, 'b': b, "fb": fb, "ff": ff, 'res': res, 'corr_img_all_r': corr_img_all_r,
               'num_list': num_list};
    if Yd_min < 0:
        Yd += Yd_min_pw;
...
def extract_pure_and_superpixels(Yd, cut_off_point=0.95, length_cut=15, length_max=100000, th=2, residual_cut=0.6,
                                 num_plane=1, patch_size=[100, 100], plot_en=False, text=False):
    """
    ...
    if num_plane > 1:
        print("3d data!");
        connect_mat_1, idx, comps, permute_col = find_superpixel_3d(Yt, num_plane, cut_off_point, length_cut,
                                                                    length_max, eight_neighbours=True);
    else:
        print("find superpixels!")
        connect_mat_1, idx, comps, permute_col = find_superpixel(Yt, cut_off_point, length_cut, length_max,
                                                                 eight_neighbours=True);
    ...
    pure_pix = np.unique(pure_pix);
    print("prepare iteration!")
    a_ini, c_ini, brightness_rank = prepare_iteration(Yd, connect_mat_1, permute_col, pure_pix, a_ini, c_ini,
                                                      more=False);
    if plot_en:
        Cnt = local_correlations_fft(Yt);
        fig = pure_superpixel_corr_compare_plot(connect_mat_1, unique_pix, pure_pix, brightness_rank_sup,
                                                brightness_rank, Cnt, text);
    else:
        Cnt = None;
    if Yd_min < 0:
        Yd += Yd_min_pw;
...
def superpixel_single_plot(connect_mat_1, unique_pix, brightness_rank_sup, text):
    ...
    if text:
        for ii in range(len(unique_pix)):
            ...
            pos0 = pos[0];
            pos1 = pos[1];
            ax.text((pos1)[np.array(len(pos1) / 3, dtype=int)], (pos0)[np.array(len(pos0) / 3, dtype=int)],
...
def pure_superpixel_corr_compare_plot(connect_mat_1, unique_pix, pure_pix, brightness_rank_sup, brightness_rank, Cnt,
                                      text=False):
    scale = np.maximum(1, (connect_mat_1.shape[1] / connect_mat_1.shape[0]));
...
def vanilla_nmf_lasso(Yd, num_component, maxiter, tol, penalty_param, c=None):
    ...
    for iters in range(maxiter):
        ...
        b = np.maximum(0, y0.mean(axis=1, keepdims=True) - (a * (c.mean(axis=0, keepdims=True))).sum(axis=1,
                                                                                                     keepdims=True));

        res[iters] = np.linalg.norm(y0 - np.matmul(a, c.T) - b, "fro") ** 2 + penalty_param * (
                abs(a).sum() + abs(c).sum());
...
def merge_components_Y(a, c, corr_img_all_r, U, normalize_factor, num_list, patch_size, merge_corr_thr=0.5,
                       merge_overlap_thr=0.8, plot_en=False):
    """ want to merge axons apatially overlap very large and temporal correlation moderately large,
	and update a and c after merge with region constrain
	Parameters:
	-----------
    ...
    cor_corr = scipy.sparse.triu(cor.T.dot(cor), k=1);
    cri = np.asarray((cor_corr / (temp.T)) > merge_overlap_thr) * np.asarray((cor_corr / temp) > merge_overlap_thr) * ((
                                                                                                                               a_corr > 0).toarray());  # .toarray())*(((cor_corr/(temp.T)) > merge_overlap_thr).toarray())*((a_corr > 0).toarray());
    a = a.toarray();
...
def update_AC_l2_Y(U, normalize_factor, a, c, b, patch_size, corr_th_fix,
    ...
    for iters in range(maxiter):
        ...
        if update_after and ((iters + 1) % update_after == 0):
            corr_img_all = vcorrcoef_Y(U / normalize_factor, c);
            rlt = merge_components_Y(a, c, corr_img_all, U, normalize_factor, num_list, patch_size,
...
def axon_pipeline_Y(Yd, fb_ini=False, ff_ini=False, cut_off_point=[0.95, 0.9], length_cut=[15, 10],
                    length_max=[100000, 100000], th=[2, 1], pass_num=1, residual_cut=[0.6, 0.6],
                    corr_th_fix=0.31, max_allow_neuron_size=0.3, merge_corr_thr=0.6, merge_overlap_thr=0.6, num_plane=1,
                    patch_size=[100, 100],
                    plot_en=False, TF=False, fudge_factor=1, text=True, bg=False, max_iter=35, max_iter_fin=50,
    ...
    while ii < pass_num:
        ...
        if ii > 0:
            ...
            a, c, b, normalize_factor, brightness_rank = prepare_iteration(Yd, connect_mat_1, permute_col, pure_pix,
                                                                           a_ini, c_ini, remove=remove, more=True);
        ...
        start_time = time.time();
        if bg:
            a, c, b, fb, ff, res, corr_img_all_r, num_list = update_AC_bg_l2_Y(Yd.reshape(np.prod(dims), -1, order="F"),
            ...
            a, c, b, fb, ff, res, corr_img_all_r, num_list = update_AC_l2_Y(Yd.reshape(np.prod(dims), -1, order="F"),
                                                                            normalize_factor, a, c, b, dims,
                                                                            corr_th_fix, maxiter=maxiter, tol=1e-8,
                                                                            update_after=update_after,
                                                                            merge_corr_thr=merge_corr_thr,
                                                                            merge_overlap_thr=merge_overlap_thr,
