...
if not os.path.isfile(out_dir + '/detr_nnorm.tif'):
    ...
    Sn_image = np.sqrt(np.reshape(psd_noise_estimate(np.reshape(mov_detr_nnorm, (fov_height * fov_width, num_frames))),
...
if not os.path.isfile(out_dir + '/denoised.tif'):
    ...
    print("Denoising (PMD) took: " + str(time.time() - start) + ' sec')
